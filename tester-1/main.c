...
int main(int argc, char **argv) {
    ...
    // Dump log
    printf("%s: tester-1 done. Dumping data...\n", __func__);
    FILE *fp = fopen("tester-1-dump.txt", "w");
    for (int i = 0; i < n_readers; ++i) {
    ...
    // Sort log by time
    system("sort tester-1-dump.txt >tester-1-dump-sorted.txt 2>/dev/null");

    // Dump raw data
    FILE *fp_db = fopen("tester-1.db", "w");
